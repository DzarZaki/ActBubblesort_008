...
void bubbleSortArray() {    // Method untuk mengurutkan array dengan metooe bubble sort
    for (int i = 1; i < n; i++) {          // Looping dengan i dimulai dari 1 hingga n-1
        for (int j = 0; j < n - i; j++) {    // Looping dengan j dimulai dari 0 hingga n-i-1
            if (a[j] > a[j + 1]) {           // Jiak nilai pada a[j] lebih besar dari a[j+]
                int temp = a[j];             // Simpan nilai a[j] ke variabel sementara temp
                a[j] = a[j + 1];             // Assign nilai a[j+1] ke a[j]
                a[j + 1] = temp;             // Assign niilai temp ke ke a[j+1]
            }
        }
    }
}
